        ...
        <div className="grid grid-cols-2 gap-4">
          {[
           ...
           <Link key={idx} href={item.link}> <div
              ...
              className="bg-white shadow-md py-4 px-2 rounded-lg flex items-center gap-2 text-black hover:scale-105 transition-all duration-300"
            >
              <span className="text-3xl">{item.icon}</span>
              <div>
                <h3 className="font-bold px-1">{item.title}</h3>
                <p className="text-sm text-gray-600 hidden sm:block">{item.desc}</p>
              </div>
            </div>
            </Link>
          ))}
